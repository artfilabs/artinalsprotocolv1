...
//         token_id: object::uid_to_inner(&user_balance.id), // Using user_balance id for initial debug
//         sender,
//         recipient: sender
//     });
    
//     // Price validation
//     assert!(price_per_nft <= MAX_PRICE, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Price exceeds maximum limit"),
//             token_id: object::uid_to_inner(&user_balance.id),
//             sender,
//             recipient: sender
//         });
//         E_MAX_PRICE_EXCEEDED
//     });
//     assert!(price_per_nft > 0, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Invalid price"),
//             token_id: object::uid_to_inner(&user_balance.id),
//             sender,
//             recipient: sender
//         });
...
//     assert!(nft_amount <= MAX_BATCH_SIZE, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Batch size exceeds maximum limit"),
//             token_id: object::uid_to_inner(&user_balance.id),
//             sender,
//             recipient: sender
//         });
//         E_MAX_BATCH_SIZE_EXCEEDED
//     });
    
//     // Verify inputs
//     let nft_length = vector::length(&nfts);
//     assert!(nft_length >= nft_amount, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Insufficient NFTs provided"),
//             token_id: object::uid_to_inner(&user_balance.id),
//             sender,
//             recipient: sender
//         });
...
//     assert!(object::uid_to_inner(&collection_cap.id) == collection_id, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Collection cap mismatch"),
//             token_id: object::uid_to_inner(&user_balance.id),
//             sender,
//             recipient: sender
//         });
...
//     // Balance validation
//     assert!(user_balance.collection_id == collection_id, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: User balance collection mismatch"),
//             token_id: object::uid_to_inner(&user_balance.id),
//             sender,
//             recipient: sender
//         });
//         E_COLLECTION_MISMATCH
//     });
//     assert!(user_balance.balance >= nft_amount, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Insufficient user balance"),
//             token_id: object::uid_to_inner(&user_balance.id),
//             sender,
//             recipient: sender
//         });
...
//     // Create sale object ID
//     let sale_id = object::new(ctx);
//     let sale_id_inner = object::uid_to_inner(&sale_id);
    
//     // Create sale object
//     let sale = NFTSale<CURRENCY> {
//         id: sale_id,
//         price_per_nft,
...
//     // Debug event for starting NFT processing
//     event::emit(DebugEvent {
//         message: string::utf8(b"Starting NFT processing"),
//         token_id: sale_id_inner,
//         sender,
//         recipient: sender
//     });
    
//     // Process NFTs and create listings
//     let mut i = 0;
//     while (i < nft_amount) {
//         let nft = vector::pop_back(&mut nfts);
//         assert!(get_collection_id(&nft) == collection_id, {
//             event::emit(DebugEvent {
//                 message: string::utf8(b"Failed: NFT collection mismatch"),
...
//         // Create NFTListing with type annotation
//         let listing: NFTListing<CURRENCY> = NFTListing {
//             id: object::new(ctx),
//             nft,
//             sale_id: sale_id_inner,
//             asset_id: i + 1,
//             price: price_per_nft
//         };
...
//         // Debug event for successful listing creation
//         event::emit(DebugEvent {
//             message: string::utf8(b"Created NFT listing"),
//             token_id: object::uid_to_inner(&listing.id),
//             sender,
//             recipient: sender
//         });
...
//         // Debug event for returning unused NFT
//         event::emit(DebugEvent {
//             message: string::utf8(b"Returning unused NFT to sender"),
//             token_id: object::uid_to_inner(&nft.id),
//             sender,
//             recipient: sender
//         });
...
//     // Update user balance using safe arithmetic
...
//     asset_id: u64,
//     listing: NFTListing<CURRENCY>, // Take ownership of the listing
//     collection_cap: &CollectionCap,
//     ctx: &mut TxContext
// ) {
...
//     // Verify listing belongs to this sale
//     assert!(listing.sale_id == object::uid_to_inner(&sale.id), E_COLLECTION_MISMATCH);
    
//     // Verify asset_id matches
//     assert!(listing.asset_id == asset_id, E_INVALID_ASSET_ID);
    
//     // Verify price
//     assert!(listing.price == sale.price_per_nft, E_INVALID_PRICE);
    
//     // Process payment
//     let payment_amount = coin::value(&payment);
//     assert!(payment_amount >= listing.price, E_INSUFFICIENT_BALANCE);
    
//     let mut payment_mut = payment;
//     let paid = coin::split(&mut payment_mut, listing.price, ctx);
...
//     // Unpack listing and transfer NFT
//     let NFTListing { id, nft, sale_id: _, asset_id: _, price: _ } = listing;
    
//     event::emit(NFTPurchased<CURRENCY> {
//         sale_id: object::uid_to_inner(&sale.id),
//         buyer,
//         nft_ids: vector[object::uid_to_inner(&nft.id)],
//         amount_paid: sale.price_per_nft
//     });
    
//     transfer::transfer(buyer_balance, buyer);
//     transfer::transfer(nft, buyer); // Transfer the NFT directly
//     object::delete(id); // Delete the listing object's ID
...
//     assert!(sender == sale.creator, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Sender is not sale creator"),
//             token_id: sale_id,
//             sender,
//             recipient: sender
//         });
...
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Sale is not active"),
//             token_id: sale_id,
//             sender,
//             recipient: sender
//         });
...
//     assert!(new_total_count <= MAX_SUPPLY, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Adding NFTs would exceed maximum supply"),
//             token_id: sale_id,
//             sender,
//             recipient: sender
//         });
...
//     // Verify collection matches
//     assert!(sale.collection_id == collection_id, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Sale collection mismatch"),
//             token_id: sale_id,
//             sender,
//             recipient: sender
//         });
...
//         // Verify each NFT belongs to the same collection
//         assert!(get_collection_id(&nft) == collection_id, {
//             event::emit(DebugEvent {
//                 message: string::utf8(b"Failed: Individual NFT collection mismatch"),
//                 token_id: object::uid_to_inner(&nft.id),
//                 sender,
//                 recipient: sender
//             });
...
//         // Debug event for successful listing creation
//         event::emit(DebugEvent {
//             message: string::utf8(b"Successfully created NFT listing"),
//             token_id: object::uid_to_inner(&listing.id),
//             sender,
//             recipient: sender
//         });
...
// // Add more NFTs to existing sale
...
//     // Initial debug event for starting process
//     event::emit(DebugEvent {
//         message: string::utf8(b"Starting remove_nfts_from_sale process"),
//         token_id: sale_id,
//         sender,
//         recipient: sender
//     });

//     // Verify sender is the sale creator
//     assert!(sender == sale.creator, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Sender is not sale creator"),
//             token_id: sale_id,
...
//     // Validate amount to remove does not exceed NFT count in sale
//     assert!(amount <= sale.nft_count, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Amount exceeds NFTs in sale"),
//             token_id: sale_id,
//             sender,
//             recipient: sender
//         });
...
//     // Ensure user_balance collection matches sale and collection cap collection
//     let collection_id = object::uid_to_inner(&collection_cap.id);
//     assert!(user_balance.collection_id == sale.collection_id && sale.collection_id == collection_id, {
//         event::emit(DebugEvent {
//             message: string::utf8(b"Failed: Collection mismatch"),
//             token_id: sale_id,
//             sender,
//             recipient: sender
//         });
...
//     // Process each NFT up to the specified amount
//     let mut removed_count = 0;
//     let mut i = 0;
//     let nft_count = vector::length(&nfts);

//     while (i < nft_count && removed_count < amount) {
//         let nft = vector::pop_back(&mut nfts);

//         // Emit event for NFT removal
//         event::emit(DebugEvent {
//             message: string::utf8(b"Removing NFT from sale"),
//             token_id: object::uid_to_inner(&nft.id),
//             sender,
//             recipient: sender
//         });

//         // Transfer removed NFT back to sender
//         transfer::public_transfer(nft, sender);
...
//     // Update sale count, user balance, and collection cap supply
//     sale.nft_count = safe_sub(sale.nft_count, removed_count);
//     user_balance.balance = safe_add(user_balance.balance, removed_count);
//     collection_cap.current_supply = safe_sub(collection_cap.current_supply, removed_count);
...
//     // Explicitly consume any remaining NFTs in the vector
...
//     // Explicitly destroy the empty vector to meet the 'drop' constraint
//     vector::destroy_empty(nfts);
// }
